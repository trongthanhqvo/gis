...
db = Db(dbname='geoapp', user=os.getenv('DATABASE_USER'), password=os.getenv('DATABASE_PASSWORD'), host='localhost', port='5432')
...
def publish_data(sender, instance, created, **kwargs):
    ...
    name = instance.name
    conn_str = os.getenv('DATABASE_POSTGRESL_URL')
 ...
 #  Publish  shp to geoserver using geoserver-rest
    ...
    geo.create_featurestore(store_name='geoApp', workspace='geoapp', db=os.getenv('DATABASE_NAME'),
                            host='localhost', pg_user=os.getenv('DATABASE_USER'), pg_password=os.getenv('DATABASE_PASSWORD'), schema='data')
    geo.publish_featurestore(workspace='geoapp', store_name='geoApp', pg_table= name)

    geo.create_outline_featurestyle(style_name='geoApp_shp', workspace='geoapp')
